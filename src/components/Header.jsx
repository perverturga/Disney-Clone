    ...
    <div className='flex items-center justify-between p-5'>
        <div className='flex items-center gap-8'> 
            ...
            <div className='hidden md:flex gap-8'>
                {menu.map((item) => (
                    <HeaderItem name={item.name} key={item.name} Icon={item.icon}
                    />
                ))}
            </div>
            
            <div className='flex md:hidden gap-5'>
                {/* index numaraları 0 dan başlar unutma burda da 0 1 2 sıradaki indexleri sıralıyor */}
                    ...
                    onClick={() => setToggle(!toggle)}>
                        ...
                        {
                            toggle ? 
                            <div className='absolute mt-3 bg-[#121212] border-[1px]
